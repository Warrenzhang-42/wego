...
admin-routes.html          # 独立 HTML 页面（无构建工具，直接浏览器打开）
  ...
  └── src/lib/admin-api.js # 管理端 SDK（持有 service_role key，绕过 RLS）
                              └── Supabase（routes / spots / home_carousel_configs 等）
```
...
### 首页轮播配置（`home_carousel_configs`）

- **表结构**：`config_key`（主键）+ `items`（JSONB 数组）+ `updated_at`。`config_key` 取值为 `general`（全站通用）或 `city:六位 adcode`（城市覆盖）。
- **契约**：`contracts/home-carousel.schema.json`（`items[]` 元素为 `type:image` 含 `image_url`，或 `type:route` 含 `route_id`）。
- **前台解析**：`api-client.getHomeCarousel(cityAdcode)` — 若存在对应 `city:` 行则**仅使用该条**（含空数组，不再回落 `general`）；否则读取 `general`。无 Supabase 或表未建时，`lib/home-carousel.js` 使用内置静态兜底图。
- **管理端**：`admin-carousel-panel.js` + `adminApi.uploadCarouselImage` / `upsertCarouselConfig` / `deleteCarouselConfig`；图片走既有 `images` Storage bucket（路径 `carousel/banners/`）。
- **RLS**：迁移 `011_home_carousel_configs.sql` — anon `SELECT` 公开，写入仅 service_role。
